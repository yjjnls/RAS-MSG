...
4. 下游应用通知可靠消息系统消息被成功消费。
5. 可靠消息系统查询该消息是否已经被所有订阅者消费完成，如果已完成，就更新`消息持久化存储部分`和`MQ`（将已经消费的消息删除），可靠消息将该消息状态更改为`已完成`。
...
步骤1-5发生错误时，会对一致性有不同的影响，此时可靠消息服务中存在消息状态为`已发送并且超时`的消息，执行以下操作：  
1. 可靠消息服务定时查询状态为已发送并超时的消息，查询消息是否存在（可从持久化部分或者MQ查询）。
2. 如果不存在，那么就删除该消息，保持数据一致性。
