...
async function subscribe(topic, service_name) {
    await client.zaddAsync(`sub:${topic}`, 0, service_name)
        .then(res => {
...
async function publish_msg(uuid) {
    await client.zrangebyscoreAsync('ras:msg', uuid, uuid)
        .then(res => {
            ...
            return client.zaddAsync(`topic:${msg.topic}`, uuid, res.toString());
...
async function fetch_msg(service_name) {
    ...
    for (var i = 0; i < sub_info.length; i = i + 2) {
        ...
        console.log(`sub topic:${topic}, max received msg id:${max_recv}`);
        await client.zrangebyscoreAsync(`topic:${topic}`, max_recv, 'inf', 'withscores')
            .then(res => {
...
async function unpublish_msg(topic, uuid) {
...
async function test() {
    try {
        ...
        // ras
        await publish_msg(msg_id1);
        await publish_msg(msg_id2);
        await publish_msg(msg_id3);
        await publish_msg(msg_id4);
        await publish_msg(msg_id5);
