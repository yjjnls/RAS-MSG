...
async function add_into_set(service_name, msg_id) {
...
async function query_from_set(service_name, msg_id) {
    let is_member = false;
...
async function subscribe(topic, service_name) {
    let is_subed = await client.zrankAsync(`sub:${topic}`, service_name)
        .then(res => {
            return res;
        })
        .error(err => {
...
async function fetch_msg(service_name) {
    ...
    for (var i = 0; i < sub_info.length; i = i + 2) {
        ...
        if (max_recv != 0) {// get one msg from every topic
            ...
            await client.zrangeAsync(`topic:${topic}`, 0, 0, 'withscores')
                .then(res => {
                    msgs = msgs.concat(res);
                })
                .error(err => {
