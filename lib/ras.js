...
function OnMessage(data, req, res) {
  ...
  if (uri.pathname == '/msg') {
    switch (req.method) {
      case 'POST':
        prepare(res, dataObj, uri.query.topic, req.headers.from);
        return;
      case 'PUT':
...
async function prepare(res, data, topic, from) {
  ...
  // create msg
  var msg_id = Date.now();
  let msg = new message(msg_id, from, topic);//default timeout 3s
  // store msg
  try {
...
async function publish(res, id) {
  ...
  try {
    ...
    await mq.publish(msg.topic, id, JSON.stringify(msg));
  } catch (error) {
...
async function consume(res, id) {
  ...
  console.log('remove msg in db according id')
  if (await mq.is_msg_consumed(msg.topic_, id)) {
    console.log(`msg[${id}] consumed!`);
    await mq.unpublish(msg.topic_, id);
    await mq.del_msg(id);
  }
  // return 200 OK
  send(res, 200, "OK");
